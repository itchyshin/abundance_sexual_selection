...
#duplication checking code:
#data %>% group_by(grid_id, ebird_COMMON_NAME,MONTH) %>% summarize(num=n()) %>% arrange(desc(num)) ->z
#z %>% filter(num>1) -> zz
#data %>% filter(ebird_COMMON_NAME %in% zz$ebird_COMMON_NAME)->zzz
#unique(zz$ebird_COMMON_NAME)

# average of 12 month - different grids
dat_ave <- data %>%
  left_join(., read_csv("Data/cleaned_body_size_data.csv") %>% 
  ...
  group_by(grid_id,ebird_COMMON_NAME,MONTH) #%>%
...
#Phylogeny = dplyr::first(TipLabel)) %>% 
  
dat_ave %<>% tidyr::nest() #unnesting for now
...
run_month <- function(month,dat_ave){
  dat_ave_month <- filter(dat_ave, MONTH==month)
...
month_list <- as.list(unique(dat_ave$MONTH))

# zz <- run_month(month="january",dat_ave)

zz <- mclapply(month_list, run_month, dat_ave, mc.cores = 18)
#saveRDS(zz,file="models_by_month.rds")

zz <- readRDS(here("Data","models_by_month.rds"))
